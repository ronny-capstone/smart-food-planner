...
const { WEEK_PATH } = require("../../frontend/src/utils/paths.jsx");
      ...
      db.get(
        ...
        async (err, profile) => {
          ...
          try {
            ...
            const weeklyPlan = await generateWeekMealPlan(
              userFilters,
              profile,
              inventory
            );
            ...
            return res.status(StatusCodes.INTERNAL_SERVER_ERROR).json({
              success: false,
              message: "Error getting weekly meal plan",
            });
          }
        }
      );
